...
const toolsListCommand = Command.make(
  "list",
  { search },
  ({ search }) =>
    Effect.gen(function* () {
    ...
    const tools = registry.listTools().filter((tool) =>
      filter ? tool.name.toLowerCase().includes(filter) : true,
    )
  ...
  Command.withDescription("List discovered Bruno tools"),
)
...
const toolsInvokeCommand = Command.make(
  ...
  Command.withDescription("Invoke one tool by name"),
)
...
const serveMcpCommand = Command.make(
  "serve-mcp",
  {},
  () =>
    Effect.gen(function* () {
  ...
  Command.withDescription("Run MCP server over stdio"),
)
